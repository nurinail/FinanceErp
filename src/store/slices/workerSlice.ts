...
export const workerSlice = createSlice({
  ...
  reducers: {
    ...
    deleteWorker:(state:WorkerInitialState,action:PayloadAction<number>)=>{
        state.workers=state.workers.filter(item=>item.id!==action.payload)
    },
    changeWorker:(state:WorkerInitialState,action:PayloadAction<WorkersType>)=>{
        const workerIndex=state.workers.findIndex(item=>item.id===action.payload.id);
